...
TYPE           = "park"  # Google Places type for parks
...
def nearby_search(lat, lng, radius):
    """
    Perform a paginated Nearby Search for parks around (lat, lng).
    """
    ...
    while True:
        ...
        if not token:
            break
        time.sleep(2)  # give Google time to activate the token
        params = {"key": API_KEY, "pagetoken": token}
    return places

# ── MAIN COLLECTION ─────────────────────────────────────────────────────────────

def collect_all_parks():
    ...
    while centers:
        ...
        # If results hit the 60-result cap and we can subdivide further, refine this cell
        ...
        for p in results:
            ...
            # Only keep true parks (exclude amusement/theme parks if desired)
...
if __name__ == "__main__":
    all_parks = collect_all_parks()
    print(f"Total parks found: {len(all_parks)}")
    # Optionally save results to CSV
    import pandas as pd
    pd.DataFrame(all_parks).to_csv("tbilisi_parks.csv", index=False)
